...
// Get product variations (children)
app.post('/api/get-variations', async (req, res) => {
  try {
    const { token, parentSKU } = req.body;

    if (!token || !parentSKU) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing token or parentSKU' 
      });
    }
    ...
    const response = await fetch(`${SC_BASE_URL}/rest/api/Catalog`, {
      method: 'POST',
      headers: {
    ...
    const variations = (data.Items || []).map(item => ({
      ...
      Size: item.Size || 'N/A',
      SKU: item.ID || item.ProductID // Keep SKU as well for reference
    }));
